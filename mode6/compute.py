...
def compute(epsilon_x, epsilon_y, gamma_xy):
    ...
    if gamma_xy == 0:
        ...
        setta_gamma_max = ((math.atan((-float(epsilon_x) + float(epsilon_y)) / float(gamma_xy))) / 2) * 180 / math.pi


    # Draw Mohr's circle by using above value
    ax2.plot([epsilon_avg, epsilon_avg], [radius, -radius], marker='o', color='blue', label='maximum shear')

    ax2.annotate((str(round(epsilon_avg, 2)) + ',' + str(round(radius, 2))), xy=(epsilon_avg, radius),
                 xytext=(epsilon_avg, 1.2 * radius), arrowprops=dict(fc='blue', lw=0.001))
    ax2.annotate((str(round(epsilon_avg, 2)) + ',' + str(round(-radius, 2))), xy=(epsilon_avg, -radius),
                 xytext=(epsilon_avg, 1.2 * (-radius)), arrowprops=dict(fc='blue', lw=0.001))
    ...
    if not os.path.isdir(BASE_DIR+'/mode6/static'):
        os.mkdir(BASE_DIR+'/mode6/static')
    else:
        # Remove old plot files
        filename = os.path.join(BASE_DIR, 'mode6', 'static', 'mode6.png')
        os.remove(filename)
    # Use time since Jan 1, 1970 in filename in order make
    # a unique filename that the browser has not chached
    figdata_png = os.path.join(BASE_DIR, 'mode6', 'static', 'mode6.png')
    plt.savefig(figdata_png)
    plt.close('all')


    return [str(round(setta_gamma_max, 3)), str(round(setta_gamma_max + 90, 3)), str(round(gamma_max, 3))]